...
class Augmentation:
    ...
    def run(
        ...
        background_color: Tuple[int] = (0, 255, 0),
        background_per_image: int = 1,
        show: bool = False,
        save: bool = False,
        ...
        if images is None:
            ...
            exit(1)
        if bboxes is None:
            self.logger.error(f"Bboxes must be provided")
            exit(1)
        if len(images) != len(bboxes):
            self.logger.error(f"Number of images and bboxes must be the same")
            exit(1)

        if all(isinstance(image, str) for image in images):
            self.logger.info(f"Loading images...")
            t_0 = time.time()
        ...
        if all(isinstance(bbox, str) for bbox in bboxes):
            self.logger.info(f"Loading bboxes...")
            t_0 = time.time()
            bboxes = [
        ...
        if resize:
            self.logger.info(f"Resizing images...")
            t_0 = time.time()
        ...
        if context:
            if backgrounds_path is None:
                ...
                exit(1)
            if background_per_image > 1:
                self.logger.error("Background per image must be greater than 1")
                exit(1)
            ...
            self.logger.info(f"Loading backgrounds...")
            t_0 = time.time()
            ...
            t_1 = time.time()
            self.logger.info(f"Backgrounds loaded in {(t_1 - t_0) * 1000:.2f} ms")

            for i in range(len(images)):
                image = images[i]
                bbox = bboxes[i]
                mask = np.all(image == background_color[::-1], axis=-1)
        ...
        if show:
            for i, output in enumerate(outputs):
                self.logger.info(
                    f"Showing image {i+1}/{len(outputs)}, press any key to continue or ESC to exit..."
                )
                self.image = output[0]
                self.bbox = output[1]
                self.__show__()
...
if __name__ == "__main__":
    ...
    augmentation.run(images=images, bboxes=bboxes, show=True)
    augmentation.run(
        context=True,
        images=[images[0]],
        bboxes=[bboxes[0]],
